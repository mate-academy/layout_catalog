...
- use product item from `product catalog__card` task and header from `moyo header` task
...
- write your code so that the small design changes (like header height or catalog__card width) require you to change only 1 value
...
- any values that depend on others (width of some smaller element, number of the catalog__cards, etc.) should be calculated with formula
...
>
>    <article data-qa="catd" class="catalog__catalog__card">
>```
